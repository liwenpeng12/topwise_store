...
public class BallScaleProgress extends View {
	...
	public BallScaleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mBallPaint.setColor(getResources().getColor(R.color.zkas_ball_color));
		...
		mStateChangeAnimListener = new RefrushAnimatorListener();
		mStateChangeAnimation.addListener(mStateChangeAnimListener);
		
		mRefushBtnBg = getResources().getDrawable(R.drawable.as_ball_progress_refushed);
		mErrorBtnBg = getResources().getDrawable(R.drawable.as_ball_progress_error);
		mDefaultBallSize = mRefushBtnBg.getIntrinsicWidth();
		
		super.setOnClickListener(new View.OnClickListener() {
			
			@Override
			public void onClick(View v) {
				if (!mIsProgressing && mOnRefreshClickListener != null && mStateDrawable == mRefushBtnBg) {
	...
	private class RefrushAnimatorListener implements AnimatorListener {
	...
	public void startProgress() {
		if (!mIsProgressing) {
			if (isAttached() && getWidth() > 0 && mStateDrawable != null) {
				...
				mStateChangeAnimListener.setState(RefrushAnimatorListener.ST_HIDE);
	...
	public void setRefrush() {
		if (mIsStateAnimate && mStateChangeAnimListener.getState() == RefrushAnimatorListener.ST_SHOW) {
			mStateDrawable = mRefushBtnBg;
			return;
		}
		...
		mStateChangeAnimListener.setState(RefrushAnimatorListener.ST_SHOW);
		mStateChangeAnimation.start();
		mIsStateAnimate = true;
	}
